...
// the map, not the pale road underneath them (HANDOVER "Offen": "Weg-Linie dominiert").
...
function paintReliefPixels(data, w, h, projector, terrain) {
  ...
  for (let py = 0; py < h; py++) {
    for (let px = 0; px < w; px++) {
      ...
      // M2b polish (HANDOVER "Offen": "Weg-Linie dominiert"): a path used to overwrite the pixel
...
function paintPrintPixels(data, w, h, projector, terrain, seed) {
  ...
  for (let py = 0; py < h; py++) {
    for (let px = 0; px < w; px++) {
      ...
      const base = mix3(PRINT_GREEN_LIGHT, PRINT_GREEN_DARK, patch > 0.55 ? clamp01((patch - 0.55) * 2.4) : 0);
      const colour = terrain.isPath(wx, wz) ? mix3(base, PATH_COLOUR, PATH_BLEND) : base;
      data[i] = clampByte(colour[0]); data[i + 1] = clampByte(colour[1]); data[i + 2] = clampByte(colour[2]); data[i + 3] = 255;
    }
  }
}
 ...
 */
export function paintRoutes(ctx, { parkDef, projector, filterCategory = null, hoverRouteId = null, style = "relief", lineWidth = 3.0, dotRadius = 3.2, numerals = false }) {
  for (const route of parkDef.routes) {
    ...
    // M2b polish (HANDOVER "Offen": "Kategorie-Farben kaum erkennbar … sättigen/glühen lassen wie im
