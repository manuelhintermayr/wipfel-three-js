...
export function addHarness(joints, m, attach) {
  ...
  addMesh(pelvis, strapRing(0.152, 0.011, T.x * 0.97, T.z * 0.99), m.webbing, { y: beltY });
  addMesh(pelvis, strapRing(0.159, 0.005, T.x * 0.97, T.z * 0.99, 1.0), m.orange, { y: beltY });
  addMesh(pelvis, new THREE.BoxGeometry(0.045, 0.03, 0.012), m.metal, { x: 0.075, y: beltY, z: 0.12, ry: 0.35 });
  addMesh(pelvis, new THREE.TorusGeometry(0.02, 0.005, 6, 18), m.metal, { y: 0.06, z: 0.135 });
  attach.hipsFront = addAnchor(pelvis, "hipsFront", 0, 0.045, 0.14);

  for (const side of [1, -1]) {
    const hip = joints[side > 0 ? "hipL" : "hipR"];
    addMesh(hip, strapRing(0.083, 0.010), m.orange, { y: -0.09 });
    addMesh(hip, strapRing(0.089, 0.004, 1, 1, 1.0), m.webbing, { y: -0.09 });
    addMesh(hip, new THREE.BoxGeometry(0.024, 0.16, 0.008), m.webbing, { y: -0.01, z: 0.076 });
  }
  ...
  for (const side of [1, -1]) {
    ...
    addMesh(chest, tubeAlong(pts, 0.012, 40), m.orange);
  }
  addMesh(chest, new THREE.BoxGeometry(0.2, 0.026, 0.008), m.webbing, { y: 1.29 - cy, z: 0.138 });   // chest connector
  addMesh(chest, new THREE.BoxGeometry(0.03, 0.03, 0.012), m.metal, { y: 1.29 - cy, z: 0.14 });
  addMesh(chest, new THREE.TorusGeometry(0.018, 0.004, 6, 16), m.metal, { y: 1.28 - cy, z: -0.14 });   // back D-ring
...
function buildCarabiner(m) {
  ...
  addMesh(g, new THREE.TorusGeometry(0.03, 0.0055, 8, 24), m.metal, { ry: HALF_PI, sx: 1, sy: 1.3, sz: 0.72 });
  addMesh(g, new THREE.CylinderGeometry(0.0045, 0.0045, 0.05, 6), m.metal, { z: 0.019, y: -0.006 });   // gate
  addMesh(g, new THREE.CylinderGeometry(0.011, 0.011, 0.012, 12), m.metal, { y: 0.028, rz: HALF_PI });
  return g;
}
...
export function addLanyard(attach, m) {
  ...
  for (const side of [1, -1]) {
    addMesh(group, tubeAlong([[0, 0, 0], [side * 0.035, -0.06, 0.045], [side * 0.058, -0.13, 0.065]], 0.006, 12), m.rope);
...
export function addTrolley(joints, m) {
  ...
  g.rotation.y = -0.2;
  joints.pelvis.add(g);
  addMesh(g, new THREE.BoxGeometry(0.01, 0.06, 0.02), m.webbing, { y: 0.06 });               // sling to the belt
  addMesh(g, new THREE.TorusGeometry(0.02, 0.004, 6, 16), m.metal, { y: 0.03, ry: HALF_PI, sy: 1.25 });
  addMesh(g, new THREE.BoxGeometry(0.02, 0.07, 0.085), m.metal, { y: -0.02 });                 // frame
  for (const side of [1, -1]) addMesh(g, new THREE.BoxGeometry(0.005, 0.075, 0.105), m.orange, { x: side * 0.014, y: -0.02 });
  for (const z of [-0.028, 0.028]) addMesh(g, new THREE.CylinderGeometry(0.03, 0.03, 0.03, 16), m.metal, { y: 0.0, z, rz: HALF_PI });
  return g;
}
